...
rad_b = asin(sin(rad_a) * (n_air/n_ice)); % 屈折角(ラジアン) 論文(3.3, 3.5)
...
rs_one = Rs(angle_a, n_air, n_ice);
disp(rs_one);

rad_b_one = asin(sin(rad_a) * (n_air/n_ice));
angle_one = rad2deg(rad_b_one); % 空気-氷 屈折角
%氷-水の反射
...
%画像保存
imwrite(result1, ['./iceThick--', num2str(ice_thick / 1000000), 'mm__degree--', num2str(angle_a), '.png']);
imshow(result1);
toc;