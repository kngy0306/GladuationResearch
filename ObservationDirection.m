%観測方向を変更しようと試みたさ
%a = 45;
%k2 = ( 1 / cos(deg2rad(a)) + 1 / sin(deg2rad(90 - a)) ) / 2;
...
%---------計算-----------
n = (n_x + n_y + n_z) / 3;
w = asin( sin(deg2rad(angle_a)) / n );

w1 = asin( sin(deg2rad(angle_a)) / sqrt(n_x^2 + n_y^2) );

w2 = 2*w - w1;

n_yz = n_y*n_z / sqrt(n_y.^2 * sin(w2).^2 + n_z.^2 * cos(w2).^2);
n_xz = n_x*n_z / sqrt(n_x.^2 * sin(w2).^2 + n_z.^2 * cos(w2).^2);
...
ice = abs((n_yz - n_xz) * (ice_thick / cos(w))); % 光路差R 論文(3.4)
...
%画像保存
imwrite(result1, ['./result_observationDirection/-iceThick--', num2str(ice_thick / 1000000), 'mm__degree--', num2str(angle_a), '.png']);
imshow(result1);
toc;